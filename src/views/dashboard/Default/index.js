...
const Dashboard = () => {
  ...
  const processAndDownloadExcelSales = (data) => {
    ...
    const gstSummary = {
      0: { basicValue: 0, sgst: 0, cgst: 0, igst: 0 },
      5: { basicValue: 0, sgst: 0, cgst: 0, igst: 0 },
      12: { basicValue: 0, sgst: 0, cgst: 0, igst: 0 },
      18: { basicValue: 0, sgst: 0, cgst: 0, igst: 0 }
    };
    ...
    const summaryTableHeader = worksheet.addRow([
      'GST%', 'BASIC VALUE', 'SGST%', 'SGST TAX', 'CGST%', 'CGST TAX', 'IGST%', 'IGST TAX', 'TOTAL TAX', 'TOTAL VALUE'
    ]);
    ...
    [0, 5, 12, 18].forEach((gst) => {
      ...
      const igst = gstSummary[gst].igst;
      const totalTax = sgst + cgst + igst;
      const totalValue = basicValue + totalTax;
  
      worksheet.addRow([
        gst, basicValue, gst / 2, sgst, gst / 2, cgst, gst, igst, totalTax, totalValue
      ]);
    });
...
const processAndDownloadExcelPurchase = (data) => {
  ...
  const tableHeader = worksheet.getRow(5);
  tableHeader.values = [
      'S.No', 'Supplier Name', 'Supplier GSTIN', 'Invoice Number', 'Invoice Date',
      'HSN Code', 'Purchase Value', 
      'GST%', 'SGST', 'CGST', 'IGST'
  ];
  tableHeader.eachCell((cell) => {
      cell.fill = {
  ...
  // Initialize totals and GST summaries
  ...
  data.forEach((item) => {
      ...
      const isTamilNadu = !supplierGSTIN || supplierGSTIN.startsWith('33');
      ...
      supplierData.sales.forEach((sale) => {
          sale.products.forEach((product) => {
              ...
              const gst = parseFloat(product.gst);
              const sgst = isTamilNadu ? gst / 2 : 0;
              const cgst = isTamilNadu ? gst / 2 : 0;
              const igst = isTamilNadu ? 0 : gst;

              // Add to totals
              totalPurchaseValue += totalWithoutGST;
              totalGST += totalWithoutGST * (gst / 100);
              totalSGST += totalWithoutGST * (sgst / 100);
              totalCGST += totalWithoutGST * (cgst / 100);
              totalIGST += totalWithoutGST * (igst / 100);

              // Update GST summary
              if (gstSummary[gst]) {
                  gstSummary[gst].basicValue += totalWithoutGST;
                  gstSummary[gst].sgst += totalWithoutGST * (sgst / 100);
                  gstSummary[gst].cgst += totalWithoutGST * (cgst / 100);
                  gstSummary[gst].igst += totalWithoutGST * (igst / 100);
              }

              worksheet.addRow([
                  sNo++, supplierName, supplierGSTIN, invoiceId, billDate,
                  hsnNumber, totalWithoutGST,
                  gst, totalWithoutGST * (sgst / 100), totalWithoutGST * (cgst / 100), totalWithoutGST * (igst / 100)
              ]);
          });
      });
  });
    ...
    // Add total row
    const totalRow = worksheet.addRow([
        '', '', '', '', '', 'Total', totalPurchaseValue, '', totalSGST, totalCGST, totalIGST
    ]);
  ...
  // Leave a few rows after the table
  worksheet.addRow([]);
  worksheet.addRow([]);
  worksheet.addRow([]);

  // Add summary header
  const summaryHeaderRow = worksheet.addRow([`SUMMARY ${month.toUpperCase()} ${year} PURCHASES`]);
  summaryHeaderRow.font = { bold: true };
  summaryHeaderRow.alignment = { vertical: 'middle', horizontal: 'center' };
  worksheet.mergeCells(summaryHeaderRow.number, 1, summaryHeaderRow.number, tableHeader.values.length);
