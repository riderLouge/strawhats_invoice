  ...
  const deleteInvoice = async () => {
    try {
      const response = await axios.delete(
        `https://api-skainvoice.top/api/invoice/delete/${selectedItem.id}`
      );
  ...
  const updateInvoiceProductQuantity = async () => {
    ...
    try {
      const response = await axios.put(
        `https://api-skainvoice.top/api/invoice/update-product/${selectedItem.id}`,
        {
        ...
        <div>
          <EditIcon
            style={{
              cursor: "pointer",
              color:
                hoveredRow === 1 && hoveredRowId === row.id
                  ? "blue"
                  : "inherit",
              display:
                currentUserRole === constants.ROLE_ADMIN ? "revert" : "none",
            }}
            onMouseEnter={() => {
              setHoveredRow(1);
          ...
          <FileCopyIcon
            style={{
              cursor: "pointer",
              color:
                hoveredRow === 2 && hoveredRowId === row.id
                  ? "blue"
                  : "inherit",
              marginLeft: "20px",
            }}
          ...
          <DeleteIcon
            style={{
              cursor: "pointer",
              color:
                hoveredRow === 3 && hoveredRowId === row.id
                  ? "blue"
                  : "inherit",
              marginLeft: "20px",
              display:
      ...
      Cell: ({ row }) => (
        <EditIcon
          style={{
            cursor: "pointer",
            color:
              hoveredRow === 1 && hoveredRowId === row.id ? "blue" : "inherit",
          }}
          onMouseEnter={() => {
            setHoveredRow(1);
            setHoveredRowId(row.id);
        ...
        <DialogTemplate
          ...
          body={
            invoiceType === "View invoice" ? (
              ...
              <Typography variant="body2">
                Are you sure do you want to delete this invoice?
              </Typography>
            ) : (
              <MaterialReactTable
                columns={editInvoiceColumns}
          ...
          type={
            invoiceType === "View invoice"
              ...
              : invoiceType === "Delete invoice"
              ? "Delete"
              : invoiceType === "Edit invoice"
              ? "Invoice"
              : "Update"
          }
          width={
            invoiceType === "Edit invoice" || invoiceType === "Edit Product"
              ? "lg"
              : "md"
          }
        />
